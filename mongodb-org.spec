...
%build
scons-2.7 --release --dbg=off MONGO_VERSION=3.4.21 CC=/usr/bin/aarch64-linux-gnu-gcc  CXX=/usr/bin/aarch64-linux-gnu-g++ TARGET_ARCH=aarch64 --disable-warnings-as-errors -j128 CFLAGS="-march=armv8-a+crc -mtune=generic" core tools VERBOSE=on

%install
scons-2.7 --release --dbg=off MONGO_VERSION=3.4.21 CC=/usr/bin/aarch64-linux-gnu-gcc  CXX=/usr/bin/aarch64-linux-gnu-g++ TARGET_ARCH=aarch64 --disable-warnings-as-errors -j128 CFLAGS="-march=armv8-a+crc -mtune=generic" install
mkdir -p $RPM_BUILD_ROOT/usr
cp -rv build/install/bin $RPM_BUILD_ROOT/usr
mv tools/bin/* $RPM_BUILD_ROOT/usr/bin
...
%doc distsrc/GNU-AGPL-3.0
%doc distsrc/README
%doc distsrc/THIRD-PARTY-NOTICES
%doc distsrc/MPL-2
